...
/*   Updated: 2022/05/21 15:00:21 by afaris           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	go_up(t_map *m)
{
	...
	if ((target == ENEMY) || (target == EXIT && m->collectibles <= 0))
		game_over(m);
...
void	go_down(t_map *m)
{
	...
	if (target != EXIT && target != WALL)
	{
		...
		m->player_pos.y += BLOC_SIZE;
		m->player_index.i++;
		render_signle_image(PLAYER_DOWN_PATH, m->mlx,
...
void	go_left(t_map *m)
{
	...
	if (target != EXIT && target != WALL)
	{
		...
		m->player_index.j--;
		render_signle_image(PLAYER_LEFT_PATH,
